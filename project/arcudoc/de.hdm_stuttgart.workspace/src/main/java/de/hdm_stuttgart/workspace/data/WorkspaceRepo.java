...
public class WorkspaceRepo {
    ...
    public void fetchMemberProjects() {
        ...
        Call<List<MemberProjectResponse>> call = supabaseRestClient.getMemberProjects(
                ApiConstants.API_KEY,
                ApiConstants.BEARER_KEY,
                "projects(title,last_updated,id,project_owners!projects_owner_id_fkey(mail,display_name)),project_role,job_label" //indicates to select title and id of project table although project_members table is queried in request (linked in supabase)
        );
